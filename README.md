...
- 在 Organization 业务上，给超级管理员分别展示组织 state 分别为0（待审核），1（通过，就是这个组织现在有哪些人），2（拒绝加入），3（开除的）的组织，记得返回id。给管理员分别展示四个state的人员。

- 在 Tweet 表上加一个创建时间字段（直接重构，用DateTime类型），完成推文的增删，在这个组织的都可以发推文，增加的时候记得判断这个人在这个组织里面，推文图片配置一下OSS服务器。只有本人/管理员可以删除。
